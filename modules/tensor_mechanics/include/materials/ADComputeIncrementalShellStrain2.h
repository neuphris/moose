...
class ADComputeIncrementalShellStrain2 : public Material
{
public:
  static InputParameters validParams();

  ADComputeIncrementalShellStrain2(const InputParameters & parameters);
...
protected:
  ...
  /// Out of plane rotation in the local normal direction
  std::vector<ADMaterialProperty<Real> *> _gamma_test;

    ///Component of out of plane rotation in the global coordinate system
  std::vector<ADMaterialProperty<Real> *> _gamma_test_x;
  std::vector<ADMaterialProperty<Real> *> _gamma_test_y;
  std::vector<ADMaterialProperty<Real> *> _gamma_test_z;
  ...
  /// Vector that stores the incremental solution at all the 24 DOFs in the 4 noded element.
  ADDenseVector _soln_vector;

  /// Vector that stores the current solution at all the 24 DOFs in the 4 noded element.
  ADDenseVector _soln_current;
  ...
  // std::vector<ADReal> _cos_xv1;
  // std::vector<ADReal> _cos_xv2;
  // std::vector<ADReal> _cos_xvn;
  // std::vector<ADReal> _cos_yv1;
  // std::vector<ADReal> _cos_yv2;
  // std::vector<ADReal> _cos_yvn;
  // std::vector<ADReal> _cos_zv1;
  // std::vector<ADReal> _cos_zv2;
  // std::vector<ADReal> _cos_zvn;

  // Angle between the global axes and local axes
  ADMaterialProperty<Real> & _cos_xv1;
  ADMaterialProperty<Real> & _cos_xv2;
  ADMaterialProperty<Real> & _cos_xvn;
  ADMaterialProperty<Real> & _cos_yv1;
  ADMaterialProperty<Real> & _cos_yv2;
  ADMaterialProperty<Real> & _cos_yvn;
  ADMaterialProperty<Real> & _cos_zv1;
  ADMaterialProperty<Real> & _cos_zv2;
  ADMaterialProperty<Real> & _cos_zvn;

  //Old value of angle between the global axes and local axes
  const MaterialProperty<Real> & _cos_xv1_old;
  const MaterialProperty<Real> & _cos_xv2_old;
  const MaterialProperty<Real> & _cos_xvn_old;
  const MaterialProperty<Real> & _cos_yv1_old;
  const MaterialProperty<Real> & _cos_yv2_old;
  const MaterialProperty<Real> & _cos_yvn_old;
  const MaterialProperty<Real> & _cos_zv1_old;
  const MaterialProperty<Real> & _cos_zv2_old;
  const MaterialProperty<Real> & _cos_zvn_old;
